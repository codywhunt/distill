...
void main() {
  ...
  group('DragSession.generatePatches', () {
    // Note: Move operations are now handled by generateDropPatches in
    // canvas_state.dart. generatePatches() only handles resize operations.

    test('returns empty for frame move (move handled by canvas_state)', () {
      const target = FrameTarget('frame_1');
      final session = DragSession.move(
        targets: {target},
        startPositions: {target: const Offset(100, 100)},
      ...
      // Move patches are generated by endDrag in canvas_state.dart, not here
      expect(patches, isEmpty);
    });

    test('returns empty for node move (move handled by canvas_state)', () {
      ...
      // Move patches are generated by generateDropPatches in canvas_state.dart
      expect(patches, isEmpty);
    });

    test('returns empty for move with nodes inside instances', () {
      const target = NodeTarget(
        frameId: 'frame_1',
